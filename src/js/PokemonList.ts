...
export const minSpeed = Math.min(...pokemonList.map(p => p.base.speed));
export const maxSpeed = Math.max(...pokemonList.map(p => p.base.speed));
...
pokemonList.forEach(p => {
  ...
  p.attack = Math.max(p.base.attack, p.base.specialAttack);
